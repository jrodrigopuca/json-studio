	...
	for (let s = 0; s < numSess; s++) {
		...
		sessions.push({
			...
			utm:
				Math.random() > 0.7
					? {
							source: rnd(utmSources),
							medium: rnd(utmMediums),
							campaign: "campaign_" + rndInt(1, 50),
						}
					: null,
		});
	}
...
const data = {
	...
	summary: {
		...
		total_mrr: parseFloat(
			users.reduce((s, u) => s + u.metadata.subscription.mrr, 0).toFixed(2),
		),
		avg_sessions: Math.round(
			users.reduce((s, u) => s + u.metadata.total_sessions, 0) / users.length,
		),
	},
	users,
};
