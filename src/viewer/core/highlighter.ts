...
export function highlightJson(json: string): string {
  return json.replace(
    ...
    (match, key?: string, str?: string, num?: string, bool?: string, nul?: string, bracket?: string, colon?: string, comma?: string) => {
      if (key) {
...
export function getTypeClass(type: string): string {
  switch (type) {
    case 'string': return TOKEN_CLASSES.string;
    case 'number': return TOKEN_CLASSES.number;
    case 'boolean': return TOKEN_CLASSES.boolean;
    case 'null': return TOKEN_CLASSES.null;
    default: return '';
  }
}
