...
export class Toolbar extends BaseComponent {
  ...
  render(container: HTMLElement): void {
    ...
    const brand = createElement('div', {
      ...
      children: [
        createElement('span', { className: 'js-toolbar__brand-icon', textContent: '⚡' }),
    ...
    // Spacer
    this.el.appendChild(createElement('div', { className: 'js-toolbar__spacer' }));
    ...
    this.on(searchBtn, 'click', () => {
      ...
      document.querySelector<HTMLInputElement>('.js-search-bar__input')?.focus();
    });
    ...
    const expandBtn = createElement('button', {
      className: 'js-toolbar__button',
      textContent: '⊞',
      attributes: { title: 'Expand all (Ctrl+Shift+F)', 'aria-label': 'Expand all nodes' },
    });
    ...
    const collapseBtn = createElement('button', {
      className: 'js-toolbar__button',
      textContent: '⊟',
      attributes: { title: 'Collapse all (Ctrl+Shift+C)', 'aria-label': 'Collapse all nodes' },
    });
    ...
    const copyBtn = createElement('button', {
      className: 'js-toolbar__button',
      textContent: '📋',
      attributes: { title: 'Copy JSON (Ctrl+C)', 'aria-label': 'Copy entire JSON' },
    });
    ...
    const themeBtn = createElement('button', {
      ...
      textContent: '🌙',
      attributes: { title: 'Toggle theme', 'aria-label': 'Toggle dark/light theme' },
    });
  ...
  update(state: Partial<AppState>): void {
    ...
    if (state.viewMode !== undefined) {
      for (const [mode, btn] of this.tabButtons) {
        btn.classList.toggle('js-toolbar__tab--active', mode === state.viewMode);
      }
    }
