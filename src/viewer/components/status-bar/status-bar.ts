...
export class StatusBar extends BaseComponent {
  ...
  render(container: HTMLElement): void {
    ...
    const spacer = createElement('span', { className: 'js-status-bar__spacer' });

    this.validEl = createElement('span', { className: 'js-status-bar__item' });

    this.el.appendChild(this.keysEl);
    this.el.appendChild(
      createElement('span', { className: 'js-status-bar__separator', textContent: '·' }),
    );
    ...
    this.watch(
      ['totalKeys', 'maxDepth', 'fileSize', 'isValid'],
      () => this.update({}),
    );
