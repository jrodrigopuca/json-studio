...
import { createElement, escapeHtml, copyToClipboard } from '../../../shared/dom.js';
import { createVirtualScroll, type VirtualScrollInstance } from '../../core/virtual-scroll.js';
...
export class TreeView extends BaseComponent {
  ...
  render(container: HTMLElement): void {
    ...
    this.watch(
      ['nodes', 'expandedNodes', 'selectedNodeId', 'searchMatches', 'searchCurrentIndex'],
  ...
  private renderTree(): void {
    ...
    if (this.visibleNodes.length === 0) {
      this.el.innerHTML = '<div style="padding: 16px; color: var(--text-secondary);">Empty JSON</div>';
      return;
    }
  ...
  private renderNode(index: number): HTMLElement {
    ...
    const row = createElement('div', {
      ...
      attributes: {
        ...
        'aria-expanded': node.isExpandable ? String(state.expandedNodes.has(node.id)) : '',
    ...
    if (node.isExpandable) {
      const toggle = createElement('button', {
        ...
        textContent: '▶',
        attributes: {
          'aria-label': state.expandedNodes.has(node.id) ? 'Collapse' : 'Expand',
        },
      });
    ...
    if (node.type === 'object') {
      ...
      row.appendChild(
        createElement('span', {
          ...
          innerHTML: node.type === 'string' ? formatStringValue(String(node.value)) : escapeHtml(valueStr),
        }),
      );
    }
  ...
  private handleKeyDown(e: KeyboardEvent): void {
    ...
    switch (e.key) {
      ...
      case 'ArrowRight': {
        ...
        if (currentNode?.isExpandable && !state.expandedNodes.has(currentNode.id)) {
      ...
      case 'ArrowLeft': {
        ...
        if (currentNode?.isExpandable && state.expandedNodes.has(currentNode.id)) {
          ...
          // Move to parent
          const parentNode = this.visibleNodes.find((n) => n.id === currentNode.parentId);
          if (parentNode) {
  ...
  private handleContextMenu(e: MouseEvent): void {
    ...
    const menu = createElement('div', { className: 'js-tree-view__context-menu' });
